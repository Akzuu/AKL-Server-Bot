 ...
 * TO BE DETERMINED
 */
const cmdStay = async (ip, port, gameState, team) => {
  if (!gameState.live && gameState.knifeWinner !== 'NONE') {
    if (gameState.knifeWinner !== team) {
      await rcon.sendMessageToAll(ip, port, ' \x10You didn\'t win knife!');
      return;
    }
  }
};
